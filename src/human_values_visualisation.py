...
def loadDataframe():
    ...
    dataset = Path(
        "Datasets/ESS-Human-Values-Cleaned.csv"
    )
...
def create_comparison_figure(df, importance_metric):
    ...
    average_importance = df_filtered.groupby(['essround', 'cntry'])['importance_score'].mean().reset_index()
    ...
    for i, country in enumerate(df['cntry'].unique()):
        country_data = average_importance[average_importance['cntry'] == country]
        country_name = country_name_map.get(country, country)
        fig.add_trace(go.Scatter(
            x=country_data['essround'],
            y=country_data['importance_score'],
            mode='lines+markers',
            name=country_name,
            hovertemplate=f"{country_name}, Round: %{{x}}, Importance: %{{y:.2f}}<extra></extra>",
            marker=dict(color=country_colors[i])  # Assign color to the line
        ))

    fig.update_layout(
        title=f"Average Importance of {importance_metric} Across All Countries",
        title_x=0.5,
...
def create_detail_figure(df, selected_country, importance_metric):
    df_country = df[df['cntry'] == selected_country].copy()
    df_country = df_country[(df_country[importance_metric] >= 1) & (df_country[importance_metric] <= 11)]

    grouped = df_country.groupby(['essround', importance_metric]).size().unstack(fill_value=0)
    ...
    for i, column in enumerate(grouped.columns):
        response_label = str(column)
        count_traces.append(go.Bar(
            x=grouped.index, 
            y=grouped[column], 
            name=response_label, 
            marker_color=bar_colors_counts[i],
            legendgroup="group1"
        ))
    ...
    for i, column in enumerate(grouped_norm.columns):
        response_label = str(column)
        proportion_traces.append(go.Bar(
            x=grouped_norm.index, 
            y=grouped_norm[column], 
            name=response_label, 
            marker_color=bar_colors_props[i],
            legendgroup="group1",
            showlegend=False
        ))
    ...
    fig = make_subplots(
        rows=2, cols=1, 
        subplot_titles=(f"{importance_metric} Distribution (Counts)", f"{importance_metric} Distribution (Proportions)"), 
    ...
    fig.update_layout(
        title_text=f"Detailed Importance Distribution in {country_name_map.get(selected_country, selected_country)} - {importance_metric}",
        title_x=0.5,
        height=800,
        barmode='stack',
        showlegend=True,
...
importance_metrics = {
    ...
    'ipsucesa': "Important to be successful and that people recognize achievements",
    'ipudrsta': "Important to understand different people"
}

importance_metric_options = [
    {'label': label, 'value': metric} 
    for metric, label in importance_metrics.items()
]

app.layout = html.Div([
    html.H1("ESS Human Values Importance Ratings Across Europe", style={'textAlign': 'center'}),
    ...
    dcc.Dropdown(
        id='importance-metric-dropdown',
        options=importance_metric_options,
        value='impdiffa',  # Default trust metric
        clearable=False,
        style={'width': '50%', 'margin': '20px auto'}
    ),
...
@app.callback(
    ...
    [Input('country-dropdown', 'value'),
     Input('importance-metric-dropdown', 'value')]
)
def update_graphs(selected_country, importance_metric):
    comparison_fig = create_comparison_figure(df, importance_metric)
    detail_fig = create_detail_figure(df, selected_country, importance_metric)
